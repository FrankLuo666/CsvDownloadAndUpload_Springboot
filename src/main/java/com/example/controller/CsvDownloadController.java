...
//                CSVPrinter csvPrinter = new CSVPrinter(writer, CSVFormat.DEFAULT);
    ...
    /* post请求到方式下载CSV */
    @PostMapping("/downloadCsvByPost")
    public ResponseEntity<Object> yourMethod(@RequestBody Product myData) {
        Map<String,String> response = new HashMap<>();
        //文件名,加时间
        SimpleDateFormat format = new SimpleDateFormat("yyyyMMddHHmmssSSS");
        String currentTime = format.format(new Date());
        String fileName = "csvFileName_" + currentTime + ".csv";
        response.put("fileName", fileName);
        //内容；在CSV文件中使用UTF-8编码，通常可以在文件头添加一行字符：\ufeff
        response.put("content","\ufeff"+"csvContent，csv内容，"+"\t"+"支持excle打开，utf-8编码。");
        return new ResponseEntity<>(response, HttpStatus.OK);
    }
